# MONyog ディスク領域 Shortage

EBSが2つattachされており、`/data` 下のスペースが減っていることがわかる
```bash
$ lsblk
...
MONyog dataの実体はSQLite database
Ref: https://wiki.idera.com/display/SQLDMYSQL/Real-Time+data
...
realtime analyticsは日毎にdatabaseを分けている模様
MONyog UIから `persited session` をLoadするときにはon-demandでDBにつないでるのだろう
不要そうなdatabaseを削除してdiskを空ければ良い
```bash
