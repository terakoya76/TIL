...
handle_interruptが呼ばれているので、stackprofのwall mode timerによって割り込まれてconnectionが終了してしまうという記事中の現象に酷似している
```
...
## Verification
straceするとprofile開始後から `SIGALRM` が飛ぶのを確認
```bash
$ sudo strace -p 10685
...
`connect` してnon-blockingにsocketがreadyになるのを待つ。
そして `EINTR` を食らい、`shutdown` されている。
これが `COUNT_HANDSHAKE_ERRORS` がincrementされる原因
...
Ruby fileを読んだりprofile結果を書き出したりする
```bash
...
tcpdumpからもhandshake後にclient側からconnectionを切っていることがわかる（seq番号がinterleaveしていて読みにくいが）
```bash
...
2. intervalをdefault値の1000usから10000usに上げる
```ruby
...
* ただこれはip blockが起きないだけで、mysql2内でのretryは防げないので性能劣化を起こす（profile結果が歪む）
