# Resolver for Route53 on VPC
VPC内でのRoute53 CNAME変更反映速度が少し気になったので調査
...
* VPC
  ...
  * DNS-enabled
* R53
  * RDS EndpointのCNAMEをmanagement consoleから変更
  * TTL1
...
## 仮説
* vpc内のstub/権威の問い合わせロジックに変なトリックがある説
* r53自体への反映遅延がある説

### Resolver 調査
同一VPCのEC2にSSH
digを見ると `127.0.0.53:53` に問い合わせており、確認してみると `systemd-resolved` に問い合わせしていることがわかる。
```bash
$ dig db.mm-test.net
...
`systemd-resolved` が問い合わせるのは `172.30.0.2:53`
今回使っているVPCのネットワークレンジが `172.30.0.0/16` ですので、Route53 VPC Resolver (`172.30.0.2`) に投げていることがわかる。
...
いったんstub-resolver側でのcacheをチェックする
Defaultは `Cache=yes` ですので、一応cache機構は動いていそう
...
結果、遅延するときは遅延したのでstub resolver cacheの問題ではなさそう
