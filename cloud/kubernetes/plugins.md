...
### iexec
easy to select pod for `kubectl exec`
```bash
$ k iexec -A flu
...
### trace
`kprobes/kretprobes, tracepoints, software, hardware, profile events` はnodeに対して。
`uprobe/uretprobe, USDT events` はpodに対してtraceを掛けるのが推奨される。
```bash
$ k trace run ip-10-1-10-231.ap-northeast-1.compute.internal -e "tracepoint:syscalls:sys_enter_* { @[probe] = count(); }"
```

header fileが見つからないケースは追加の依存をnodeに入れる必要あり
```bash
